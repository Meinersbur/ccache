...
EOF
    ...
    if [[ "${CCACHE_SLOPPINESS}" = *"incbin"* ]]; then
        ...
        expect_print_stats 'unsupported_code_directive' 0
        expect_stat 'cache hit (preprocessed)' 0
        expect_stat 'cache miss' 1
        ...
        expect_print_stats 'unsupported_code_directive' 1
    fi
...
EOF
    ...
    if [[ -n "$CCACHE_NOCPP2" && "${CCACHE_SLOPPINESS}" != *"unify_with_output"*  ]]; then
        # Any output produces a fallback in unify+nocpp2 mode; diagnostics would refer to temporary file
        expect_print_stats 'compiler_produced_stderr' 1
        expect_stat 'cache miss' 0
    else
        expect_print_stats 'compiler_produced_stderr' 0
        expect_stat 'cache miss' 1

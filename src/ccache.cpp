...
process_preprocessed_file(Context& ctx,
                          ...
                          bool pump)
{
  bool detect_incbin = !(ctx.config.sloppiness() & SLOPPY_INCBIN);
  ...
  while (q < end - 7) {
    ...
    } else if (detect_incbin && q[0] == '.' && q[1] == 'i' && q[2] == 'n'
               && q[3] == 'c' && q[4] == 'b' && q[5] == 'i' && q[6] == 'n') {
...
static bool
hash_deps_from_depfiles(Context& ctx, Hash* hash, const std::string& depfile)
{
  std::string file_content;
  try {
    file_content = Util::read_file(depfile);
  } catch (const Error& e) {
    LOG("Cannot open dependency file {}: {}", depfile, e.what());
    return false;
  }
...
static bool
hash_depfiles(Context& ctx,
              Hash* direct_hash,
              Hash* pch_hash,
              const std::string& depfile)
{
  if (ctx.config.direct_mode() && !depfile.empty()
      && !hash_deps_from_depfiles(ctx, direct_hash, depfile)) {
    return false;
  }
...
{
  if (!hash_depfiles(ctx, &hash, &hash, ctx.args_info.output_dep)) {
    return nullopt;
  }
...
// Process the output of the preprocessor output in unify mode.
static Statistic
process_unified_file(Context& ctx,
                     Hash& hash,
                     const std::string& stdout_path,
                     const std::string& deps_path)
{
  // Include the dependency file into the hash. There may be no dependency file
  // if directly processing an .i file, in which case the input file is the only
  // dependency.
  if (!deps_path.empty() && !hash.hash_file(deps_path)) {
    LOG("Failed to open {}: {}", stdout_path, strerror(errno));
    return Statistic::internal_error;
  }

  // The preprocessed file already contains all the content, so throw away the
  // included files' hashes, but still record them for direct mode.
  if (!hash_depfiles(ctx, nullptr, &hash, deps_path)) {
  ...
  if (ctx.config.sloppiness() & SLOPPY_INCBIN) {
    ...
    std::string data;
    try {
      data = Util::read_file(stdout_path);
    } catch (Error& err) {
      LOG("Failed to open {}: {}", stdout_path, err.what());
      return Statistic::internal_error;
    }
         ...
         Hash* depend_mode_hash)
{
  ...
  if (ctx.config.unify_mode()
      && !(ctx.config.sloppiness() & SLOPPY_UNIFY_WITH_OUTPUT)) {
    ...
    if (se.size() != 0) {
      ...
      // compiler messages in the hash. This will guarantee that there will
      // never be a match in preprocessor mode, but the file can still be
      // referred to from the manifest, and thus, by direct mode.
      LOG_RAW("Invalidate preprocessed output hash");
      depend_mode_hash->hash_delimiter("diag");
      std::string stderr_content = Util::read_file(tmp_stderr_path);
      depend_mode_hash->hash(stderr_content);
      ctx.set_result_name(depend_mode_hash->digest());
    }
  }
...
get_result_name_from_cpp(Context& ctx, Args& args, Hash& hash)
{
  ...
  if (ctx.args_info.direct_i_file) {
    ...
    TemporaryFile tmp_deps(FMT("{}/tmp.cpp_deps", ctx.config.temporary_dir()));
    ctx.register_pending_tmp_file(tmp_deps.path);

    size_t args_added = 2;
    args.push_back("-E");

    // Use -fnormalize-whitespace if available. Outside unify mode, this will
    // still normalize tokens on this same line.
    if (ctx.config.compiler_type()
        == CompilerType::clang_normalize_whitespace) {
      args.push_back("-fnormalize-whitespace");
      args_added += 1;
    }

    if (ctx.config.unify_mode()) {
      // -P suppresses line directives in the preprocessed output, hence cannot
      // be used to track dependencies anymore.
      args.push_back("-P");
      args_added += 1;

      if (ctx.args_info.generating_dependencies
          && !ctx.config.run_second_cpp()) {
        // User already specified they want to generate dependencies; Since
        // without run_second_cpp only the preprocessor run can generate a
        // dependency file, use the same for both purposes.
        deps_path = ctx.args_info.output_dep;
        LOG("Unify mode: Using user dependency file {}", deps_path);
      } else {
        // Add our own dependency output file for dependency tracking
        deps_path = tmp_deps.path;
        LOG("Unify mode: Using temporary dependency file {}", deps_path);
        args.push_back(
          (ctx.config.sloppiness() & SLOPPY_SYSTEM_HEADERS) ? "-MMD" : "-MD");
        args.push_back("-MF");
        args.push_back(deps_path);
        args_added += 3;
      }
    }
  ...
  if (ctx.config.unify_mode()) {
    const Statistic error =
      process_unified_file(ctx, hash, stdout_path, deps_path);
...
{
  return !args_info.dependency_target_specified
         && (args_info.seen_MD || args_info.seen_MMD);
}
                      ...
                      bool direct_mode)
{
  ...
  // those arguments.
  int is_clang = ctx.config.base_compiler_type() == CompilerType::clang
                 || ctx.config.base_compiler_type() == CompilerType::other;
...

  {
    ...
    if (!preprocessor.empty()) {
      if (!Util::is_full_path(preprocessor)) {
        ...
        if (resolved_compiler.empty()) {
          throw Fatal("Could not find preprocessor \"{}\" in PATH",
                      preprocessor);
        }
      }
    }
...
do_cache_compilation(Context& ctx, const char* const* argv)
{
  ...
  // In depend_ and unify mode, extend the direct hash.
  Hash* depend_mode_hash = (ctx.config.depend_mode() || ctx.config.unify_mode())
                             ? &direct_hash
                             : nullptr;
